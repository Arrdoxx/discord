...
def search(url):
    try:
        ...
        session.get("https://www.vinted.fr/", headers=headers)
        cookies = session.cookies.get_dict()
        print(cookies)
        reponse = session.get(str(url), headers=headers, cookies=cookies)
        if 429 == reponse.status_code:
            print(f"{Spy.blanc}[{Spy.rouge}ERREUR{Spy.blanc}] - Rate Limit !")
            time.sleep(60)
        soup = BeautifulSoup(reponse.text, "html.parser")
            ...
            try:
                ...
                for i in range(9, -1, -1):
                    ...
                    if str(post) in posting:
                        ...
                        data = {"username": "OxBot",
                                "avatar_url": "https://cdn.discordapp.com/attachments/1059540261769842699/1069280278117359646/Sans_titre-1.png",
                                "embeds": [
                                    {
